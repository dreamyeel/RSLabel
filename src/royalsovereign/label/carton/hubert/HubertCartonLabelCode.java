...
public class HubertCartonLabelCode {
	boolean check;
	String output;
	int cartonCount;
	
	public HubertCartonLabelCode() {
	}
		...
		try {
			...
			ResultSet rs = stmt
					.executeQuery("SELECT soh.SalesOrderNo, soh.ShipToName, soh.ShipToAddress1, " +
							...
							"soh.CustomerPONo, 	sod.CommentText, sod.ItemCodeDesc, sod.ItemCode, " + 
							...
							"Where soh.SalesOrderNo='" + soNum + "' AND sod.ItemCode <> '\\*'");			
	...
	private void generateCSV(String output) {
		String fileLocation = "//10.0.0.10/Public/IT/Label_Printing/bartender_edi/HubertCartonLabel.csv";
		try {
